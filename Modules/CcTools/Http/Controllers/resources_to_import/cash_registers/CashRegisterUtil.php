...
class CashRegisterUtil extends Util
{
    ...
    public function getRegisterDetails($register_id = null)
    {
        ...
        $register_details = $query->select(
			...
			'cash_registers.declaration',
			'cash_registers.personal_check',
            'cash_registers.closing_amount',
            'cash_registers.location_id',
            'cash_registers.denominations',
			'cash_registers.fondo_final',
			'cash_registers.fondo_initial',
			'cash_registers.total_card_slips',
			'cash_registers.total_card_amount',
			'cash_registers.total_cheques',
			'cash_registers.total_cheque_amount',
			'cash_registers.total_other_payment_amount',
			'cash_registers.total_other_payment_mod',
			'cash_registers.banks_transfers_amount',
			'cash_registers.banks_transfers',
		...
		$register_details['total_all_other_pay']=
			$register_details->total_other+
			$register_details->total_advance+
			$register_details->total_custom_pay_1+
			$register_details->total_custom_pay_2+
			$register_details->total_custom_pay_3+
			$register_details->total_custom_pay_4+
			$register_details->total_custom_pay_5+
			$register_details->total_custom_pay_6+
			$register_details->total_custom_pay_7;
			
		
		$register_details['total_all_pay_refund']=
			$register_details->total_cash_refund+
			$register_details->total_cheque_refund+
			$register_details->total_card_refund+
			$register_details->total_bank_transfer_refund+
			$register_details->total_advance_refund+
			$register_details->total_other_refund+
			$register_details->total_custom_pay_1_refund+
			$register_details->total_custom_pay_2_refund+
			$register_details->total_custom_pay_3_refund+
			$register_details->total_custom_pay_4_refund+
			$register_details->total_custom_pay_5_refund+
			$register_details->total_custom_pay_6_refund+
			$register_details->total_custom_pay_7_refund;
			
		$register_details['cash_by_system']= $register_details->total_cash - $register_details->total_cash_expense;
		$register_details['totals_of_all_sells']=	
			$register_details->total_cash +
			$register_details->total_cheque +
			$register_details->total_card +
			$register_details->total_bank_transfer +
			$register_details->total_all_other_pay;
			
		$register_details['output'] = 'Hola';
	...
	//MODIFICACIONES
	public function getRegisterTransactionDetailsCtgy($user_id, $open_time, $close_time, $is_types_of_service_enabled = false)
    {
        ...
        $product_details_ctgy = Transaction::where('transactions.created_by', $user_id)
                ->whereBetween('transactions.created_at', [$open_time, $close_time])
                ->where('transactions.type', 'sell')
                ->where('transactions.status', 'final')
                ->where('transactions.is_direct_sale', 0)
        ...
        //If types of service
        $types_of_service_details_ctgy = null;
        if ($is_types_of_service_enabled) {
            $types_of_service_details_ctgy = Transaction::where('transactions.created_by', $user_id)
                ->whereBetween('transaction_date', [$open_time, $close_time])
                ->where('transactions.is_direct_sale', 0)
                ->where('transactions.type', 'sell')
                ->where('transactions.status', 'final')
        ...
        $transaction_details_ctgy = Transaction::where('transactions.created_by', $user_id)
                ->whereBetween('transactions.created_at', [$open_time, $close_time])
                ->where('transactions.type', 'sell')
                ->where('transactions.is_direct_sale', 0)
                ->where('transactions.status', 'final')
        ...
        return ['product_details_by_category' => $product_details_by_category,
            'transaction_details_ctgy' => $transaction_details_ctgy,
            'types_of_service_details_ctgy' => $types_of_service_details_ctgy,
            'product_details_ctgy' => $product_details_ctgy,
        ];
    }
	
	
	public function getRegisterTransactionDetailsExpense($user_id, $open_time, $close_time)
	{
		$expense_details_by_category = Transaction::where('transactions.created_by', $user_id)
			->whereBetween('transactions.created_at', [$open_time, $close_time])
			->where('transactions.type', 'expense')
			->leftJoin('expense_categories AS EC', 'transactions.expense_category_id', '=', 'EC.id') // Cambio de join a leftJoin
			->select(
				'EC.name as expense_name',
				'additional_notes as expenses_note',
				DB::raw('final_total as total_expenses')
			)
			->orderByRaw('CASE WHEN expense_name IS NULL THEN 2 ELSE 1 END, expense_name')
			->get();

		return ['expense_details_by_category' => $expense_details_by_category];
	}

	
	public function getRegisterTransactionDetailsDiscounts($user_id, $open_time, $close_time)
    {
        $discount_details_by_category = Transaction::where('transactions.created_by', $user_id)
                ->whereBetween('transactions.created_at', [$open_time, $close_time])
                ->where('transactions.type', 'sell')
				->where('transactions.discount_amount', '!=', 0)
				->join('contacts AS CNT', 'transactions.contact_id', '=', 'CNT.id')
                ->select(
					'CNT.name as name_cnt',
					DB::raw('SUM(total_before_tax) as total_discounts'),
					DB::raw('invoice_no as invoice'),
					DB::raw('additional_notes as note'),
                )
                ->get();

        return ['discount_details_by_category' => $discount_details_by_category,
        ];
    }
	
	
	
	public function getRegisterTransactionDetailsCustomPay($user_id, $open_time, $close_time)
    {
        $details_custom_payment = Transaction::where('transactions.created_by', $user_id)
                ->whereBetween('transactions.created_at', [$open_time, $close_time])
                ->where('transactions.type', 'sell')
				->where('transactions.discount_amount', '!=', 0)
				->join('contacts AS CNT', 'transactions.contact_id', '=', 'CNT.id')
				->leftjoin('cash_register_transactions AS CRT', 'transactions.id', '=', 'CRT.transaction_id')
                ->select(
					'CNT.name as name_cnt',
					'CRT.pay_method as payment_m',
					'CRT.amount as amount_pay',
					DB::raw('SUM(total_before_tax) as total_discounts'),
					DB::raw('invoice_no as invoice'),
					DB::raw('additional_notes as note'),
                )
        ...
        return ['details_custom_payment' => $details_custom_payment,
        ];
    }
//MODIFICACIONES
