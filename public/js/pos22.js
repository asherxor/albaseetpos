...
$(document).ready(function() {
    ...
    if ($('#search_product').length) {
        ...
        $('#search_product')
			...
			// hero70411 - fix loi hien thi gia
            if (item.enable_stock == 1 && item.qty_available <= 0 && !is_overselling_allowed && !for_so && !is_draft) {
                ...
                string +=
                    ...
                    ')' +
                    '<br> Giá: ' +
                    __currency_trans_from_en(selling_price, true) +
                ...
                string += ' (' + item.sub_sku + ')' + '<br> Giá: ' + __currency_trans_from_en(selling_price, true);
                if (item.enable_stock == 1) {
                    var qty_available = __currency_trans_from_en(item.qty_available, false, false, __currency_precision, true);
                    string += ' (Tồn kho hiện tại: ' + qty_available + ' ' + item.unit + ')';
                }
                string += '</div>';
...
// on click sub category in category drawer
    ...
    $('.main-category').on('click', function(){
        ...
        if (parent == 0) {
            ...
            $('.category_heading').text('Danh mục con: ' + $(this).data('name'));
    ...
    $('.category-back').on('click', function(){
        ...
        $('.category-back').hide();
        $('.category_heading').text('Danh mục');
    });
